...
class Player(pg.sprite.Sprite):
    def __init__(self, game):
        ...
        self.game = game
        self.x, self.y = self.game.Level.player_pos[0], self.game.Level.player_pos[1]
    ...
    def rotate_toward_mouse(self):
        ...
        world_mouse_x = (mouse_px + self.game.camera_x) / self.game.tile_size
        world_mouse_y = (mouse_py + self.game.camera_y) / self.game.tile_size
    ...
    def update_hitboxes(self):
        ...
        center_x_world = (self.x + math.cos(self.angle) * self.hand_lado/32) * self.game.tile_size
        center_y_world = (self.y + math.sin(self.angle) * self.hand_lado/32) * self.game.tile_size
        self.rect_hand_world = self.roteted_hand.get_rect(center=(center_x_world, center_y_world))

        center_x_world = (self.x + math.cos(self.angle) * self.hit_lado/32) * self.game.tile_size
        center_y_world = (self.y + math.sin(self.angle) * self.hit_lado/32) * self.game.tile_size
    ...
    def draw(self):
        ...
        pg.draw.line(self.game.screen, 'yellow',
                     ...
                     (self.x * self.game.tile_size - cam_x + ANCHO/4 * math.cos(self.angle),
                      self.y * self.game.tile_size - cam_y + ANCHO/4 * math.sin(self.angle)), 2)
    ...
    def create_hand(self):
        self.hand_lado = 1.2 * self.game.tile_size
